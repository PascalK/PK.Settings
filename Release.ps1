...
Write-Host $Root
$Solution = Get-ChildItem $Root "PK.Settings.sln" -File
...
$PKSettings = Get-ChildItem $Sources "PK.Settings" -Directory
Write-Host $PKSettings.FullName
$PKSettingsProj = Get-ChildItem $PKSettings.FullName "PK.Settings.csproj" -File
Write-Host $PKSettingsProj.FullName

#goto :eof
& $NuGet restore $Solution.FullName

& msbuild /t:Pack /p:Configuration=Release $PKSettingsProj.FullName /p:NuGetExePath=$NuGet